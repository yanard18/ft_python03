...
if __name__ == "__main__":
    ...
    for arg in args:
        ...
        try:
            ...
            if val <= 0:
                raise Exception(
                    f"Error - for item '{key}' value must be greater than 0"
                )
            inventory[key] = val
    ...
    if inventory:
        ...
        min_key = keys[0]
        for k in keys:
            if inventory[k] > inventory[max_key]:
                max_key = k
            if inventory[k] < inventory[min_key]:
                min_key = k

        print("Item most abundant:" +
              f" {max_key} with quantity {inventory[max_key]}"
              )
        print("Item least abundant:" +
              f" {min_key} with quantity {inventory[min_key]}"
              )
